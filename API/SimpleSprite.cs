...
namespace API
{
	...
	public class SimpleSprite
	{
		...
		public float Width 
		{
			get { return this.width * Scale.X;}
			set { this.width = value; }
		}
		/// <summary>スプライトの高さ。</summary>
		public float Height 
		{
			get { return this.height * Scale.Y;}
		...
		public SimpleSprite(GraphicsContext graphics, Texture2D texture, int x0, int y0, int x1, int y1,
		                    float width, float height, float rotation, Vector2 position)
		{
			if(shaderProgram == null)
			{
				shaderProgram=CreateSimpleSpriteShader();
			...
			this.SetTextureCoord(x0,y0,x1,y1);
			this.Width = width;
			this.Height = height;
			this.Rotation = (180.0f+rotation)/180.0f*FMath.PI;
		...
		public void Render()
		{
			...
			else
			{
				...
				rc=FMath.Cos(Rotation);
				rs=FMath.Sin(Rotation);
				
				x=-Width*Center.X;
				y=-Height*Center.Y;
				vertices[0]=Position.X + (float)(x*rc - y*rs);	// x0
				vertices[1]=Position.Y + (float)(x*rs + y*rc);	// y0
				vertices[2]=Position.Z;						// z0

				x=-Width*Center.X;
				y=Height*(1.0f-Center.Y);
				vertices[3]=Position.X + (float)(x*rc - y*rs);	// x1
				vertices[4]=Position.Y + (float)(x*rs + y*rc);// y1
				vertices[5]=Position.Z;						// z1

				x=Width*(1.0f-Center.X);
				y=-Height*Center.Y;
				vertices[6]=Position.X + (float)(x*rc-y*rs);	// x2
				vertices[7]=Position.Y + (float)(x*rs+y*rc);		// y2
				vertices[8]=Position.Z;							// z2

				x=Width*(1.0f-Center.X);
				y=Height*(1.0f-Center.Y);
				vertices[9]=Position.X + (float)(x*rc-y*rs);	// x3
				vertices[10]=Position.Y + (float)(x*rs+y*rc);// y3
				vertices[11]=Position.Z;					// z3
			}
		...
		{
			string resourceName = "API.shaders.SimpleSprite.cgx";

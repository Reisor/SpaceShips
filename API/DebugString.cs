...
namespace API
{
	...
	public class DebugString  : IDisposable
	{
		...
		int maxNumOfCharactor=256; /**< Maximum number of characters.*/
		...
		{
			get { return this.rotate90; }
			set { this.rotate90 = value; }
		}
		...
		public DebugString(GraphicsContext graphics, Texture2D texture, int charaWidth, int charaHeight)
		{
			...
			if( texture != null)
				...
				this.texture = CreateTextureFromEmbeddedFile("API.resources.DebugFont.png");
		...
		public void Render()
		{
			...
			for (int i = 0; i < stringBuilder.Length * 4 * 3; i += 12)
			{
				...
				else
				{
					...
					charaPositions[i] = offsetX;
					charaPositions[i + 1] = offsetY + width;
					charaPositions[i + 2] = PixelScreenPosition.Z;

					charaPositions[i + 3] = offsetX + height;
					charaPositions[i + 4] = offsetY + width;
					charaPositions[i + 5] = PixelScreenPosition.Z;

					charaPositions[i + 6] = offsetX;
					charaPositions[i + 7] = offsetY;
					charaPositions[i + 8] = PixelScreenPosition.Z;

					charaPositions[i + 9] = offsetX + height;
					charaPositions[i + 10]= offsetY;
					charaPositions[i + 11]= PixelScreenPosition.Z;
				}
		...
		{
			Byte[] dataBuffer = Utility.ReadEmbeddedFile("API.shaders.SimpleSprite.cgx");
